 ...
 */
final class Version20250414164748 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        // Create assignment table that depends on car
        $this->addSql(<<<'SQL'
            CREATE TABLE assignment (idAssignment INT AUTO_INCREMENT NOT NULL, descriptionAssignment VARCHAR(500) NOT NULL, statusAssignment VARCHAR(30) NOT NULL, dateAssignment DATETIME NOT NULL, idCar INT NOT NULL, INDEX IDX_30C544BA5675FB1A (idCar), PRIMARY KEY(idAssignment)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB
        SQL);

        // Create assignment_mechanics table that depends on both assignment and mechanic
        $this->addSql(<<<'SQL'
            CREATE TABLE assignment_mechanics (idAssignment INT NOT NULL, idMechanic INT NOT NULL, INDEX IDX_FFB535AAD19302F8 (idAssignment), INDEX IDX_FFB535AA9A67DB00 (idMechanic), PRIMARY KEY(idAssignment, idMechanic)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB
        SQL);

        // Add foreign key constraints
        $this->addSql(<<<'SQL'
            ALTER TABLE assignment ADD CONSTRAINT FK_30C544BA5675FB1A FOREIGN KEY (idCar) REFERENCES car (idCar)
        SQL);

        $this->addSql(<<<'SQL'
            ALTER TABLE assignment_mechanics ADD CONSTRAINT FK_FFB535AAD19302F8 FOREIGN KEY (idAssignment) REFERENCES assignment (idAssignment)
        SQL);

        $this->addSql(<<<'SQL'
            ALTER TABLE assignment_mechanics ADD CONSTRAINT FK_FFB535AA9A67DB00 FOREIGN KEY (idMechanic) REFERENCES mechanic (idMechanic)
        SQL);
        ...
        $this->addSql(<<<'SQL'
            CREATE TABLE response (id_response INT NOT NULL, content_response LONGTEXT NOT NULL, date_response DATETIME NOT NULL, idUser INT DEFAULT NULL, idSubmission INT DEFAULT NULL, INDEX IDX_3E7B0BFBFE6E88D7 (idUser), INDEX IDX_3E7B0BFBF5E9DB0D (idSubmission), PRIMARY KEY(id_response)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB
        SQL);
        ...
        $this->addSql(<<<'SQL'
            ALTER TABLE item ADD CONSTRAINT FK_1F1B251EFA237437 FOREIGN KEY (orderId) REFERENCES `order` (id_order)
        SQL);

        $this->addSql(<<<'SQL'
            ALTER TABLE response ADD CONSTRAINT FK_3E7B0BFBFE6E88D7 FOREIGN KEY (idUser) REFERENCES user (id_user)
        SQL);

        $this->addSql(<<<'SQL'
            ALTER TABLE response ADD CONSTRAINT FK_3E7B0BFBF5E9DB0D FOREIGN KEY (idSubmission) REFERENCES submission (id_submission)
        SQL);

        $this->addSql(<<<'SQL'
            ALTER TABLE submission ADD CONSTRAINT FK_DB055AF3FE6E88D7 FOREIGN KEY (idUser) REFERENCES user (id_user)
        SQL);
    }
    ...
    public function down(Schema $schema): void
    {
        ...
        SQL);
        $this->addSql(<<<'SQL'
            ALTER TABLE assignment_mechanics DROP FOREIGN KEY FK_FFB535AAD19302F8
        SQL);
        $this->addSql(<<<'SQL'
            ALTER TABLE assignment_mechanics DROP FOREIGN KEY FK_FFB535AA9A67DB00
        SQL);
        $this->addSql(<<<'SQL'
        ...
        SQL);
        $this->addSql(<<<'SQL'
            ALTER TABLE response DROP FOREIGN KEY FK_3E7B0BFBF5E9DB0D
        SQL);
        $this->addSql(<<<'SQL'
