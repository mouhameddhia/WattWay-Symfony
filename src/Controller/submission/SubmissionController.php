    ...
    public function delete(Request $request, Submission $submission, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('delete'.$submission->getIdSubmission(), $request->request->get('_token'))) {
            try {
                ...
                $entityManager->flush();
                $this->addFlash('success', 'Submission removed successfully');
            } catch (\Exception $e) {
                $this->addFlash('error', 'Please try again in a moment');
            }
        } else {
            $this->addFlash('error', 'Please refresh the page and try again');
        }
