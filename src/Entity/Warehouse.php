<?php

namespace App\Entity;

use Doctrine\ORM\Mapping as ORM;
use Doctrine\Common\Collections\ArrayCollection;
use Doctrine\Common\Collections\Collection;

use App\Repository\WarehouseRepository;

#[ORM\Entity(repositoryClass: WarehouseRepository::class)]
#[ORM\Table(name: 'warehouse')]
class Warehouse
{
    #[ORM\Id]
    #[ORM\GeneratedValue]
    #[ORM\Column(name:'idWarehouse',type: 'integer')]
    private ?int $idWarehouse = null;

    #[ORM\Column(name:'nameWarehouse',type: 'string', length: 255)]
    private ?string $nameWarehouse = null;

    #[ORM\Column(name:'locationWarehouse', type: 'string', length: 255)]
    private ?string $locationWarehouse = null;

    #[ORM\Column(name:'capacityWarehouse',type: 'integer')]
    private ?int $capacityWarehouse = null;

    public function getIdWarehouse(): ?int
    {
        return $this->idWarehouse;
    }

    public function setIdWarehouse(int $idWarehouse): self
    {
        $this->idWarehouse = $idWarehouse;
        return $this;
    }

    public function getCapacityWarehouse(): ?int
    {
        return $this->capacityWarehouse;
    }

    public function setCapacityWarehouse(int $capacityWarehouse): self
    {
        $this->capacityWarehouse = $capacityWarehouse;
        return $this;
    }

    #[ORM\Column(name:'city', type: 'string', nullable: false)]
    private ?string $city = null;

    public function getCity(): ?string
    {
        return $this->city;
    }

    public function setCity(string $city): self
    {
        $this->city = $city;
        return $this;
    }

    #[ORM\Column(name:'street', type: 'string', nullable: false)]
    private ?string $street = null;

    public function getStreet(): ?string
    {
        return $this->street;
    }

    public function setStreet(string $street): self
    {
        $this->street = $street;
        return $this;
    }

    #[ORM\Column(name:'postalCode',type: 'string', nullable: false)]
    private ?string $postalCode = null;

    public function getPostalCode(): ?string
    {
        return $this->postalCode;
    }

    public function setPostalCode(string $postalCode): self
    {
        $this->postalCode = $postalCode;
        return $this;
    }

}
